    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        ...
        >
          ...
          <span className="hidden xl:block text-sm font-medium text-neutral-900 dark:text-white max-w-[120px] truncate">
            {name}
          </span>
      ...
      <DropdownMenuContent
        align="end"
        className="w-60 max-w-xs rounded-xl shadow-xl border bg-popover p-0 mt-2"
      >
        <div className="flex flex-col items-center gap-2 px-4 py-4 bg-muted rounded-t-xl border-b">
          ...
          <div className="text-center w-full">
            ...
            {formatCRM() && (
              <p className="text-xs text-muted-foreground truncate">{formatCRM()}</p>
            )}
            {specialty && (
              <p className="text-xs text-muted-foreground truncate mt-1">{specialty}</p>
            )}
          </div>
        </div>
        ...
        <div className="py-1">
          <DropdownMenuItem
            asChild
            className="transition-colors duration-100 hover:bg-muted/80 focus:bg-muted/80 cursor-pointer flex items-center gap-2 px-4 py-2"
            role="menuitem"
          >
            <Link to="/profile">
