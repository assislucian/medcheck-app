...
def test_parse_demonstrativo_main(pdf_path, cbhpm_path):
    ...
    assert (
        len(df) > 0
    ), "O DataFrame do demonstrativo está vazio, mas deveria conter procedimentos."
    # Todos os códigos extraídos devem estar na CBHPM
    cbhpm_codes = set(pd.read_excel(cbhpm_path)["codigo"].astype(int))
    assert set(df["codigo"]).issubset(
        cbhpm_codes
    ), "Existem códigos no demonstrativo que não estão na CBHPM."
    # O parser não deve retornar linhas para PDFs sem procedimentos válidos
    # (Teste negativo: arquivo fictício)
    empty_df = (
        parse_demonstrativo(
            "data/demonstrativos/arquivo_inexistente.pdf", user_crm="6091"
        )
        if os.path.exists("data/demonstrativos/arquivo_inexistente.pdf")
        else pd.DataFrame()
    )
...
def test_upload_guia_endpoint(client, jwt_token):
    ...
    with tempfile.NamedTemporaryFile(suffix=".pdf") as tmp:
        ...
        assert response.status_code in (
            ...
            400,
            422,
        )  # 422 é esperado para payload inválido
        if response.status_code == 200:
            ...
            assert (
                "Erro ao processar guia" in response.text
                or "Apenas arquivos PDF" in response.text
