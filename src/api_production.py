...
@app.get("/")
async def root():
    return {"message": "MedCheck API - Produção Render", "version": "1.0.0", "environment": ENVIRONMENT, "docs": "/docs", "status": "operational"}
...
async def login_for_access_token(
    ...
    """Login e geração de token com verificação de UF."""
    ...
    # Verificar UF enviada (se enviada) contra UF do usuário
...
async def password_recovery(request: PasswordRecoveryRequest, db: Session = Depends(get_db)):
    ...
    if not user:
        return JSONResponse(status_code=200, content={"message": "Se um usuário com este e-mail existir, um link de recuperação será enviado."})

    reset_token = create_access_token(data={"sub": user.crm, "type": "password_reset"}, expires_delta=timedelta(minutes=15))
    logger.info(f"🔑 Token de reset gerado para {user.email}: {reset_token}")
    return {"message": "Link de recuperação enviado (simulado).", "reset_token": reset_token}
