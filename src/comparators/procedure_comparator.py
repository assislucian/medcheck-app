...
class ProcedureComparator:
    def __init__(
        self,
        cbhpm_parser: CBHPMParser,
        guia_parser,
        demonstrativo_parser: DemonstrativoParser,
    ):
        """
        ...
        for procedure in guide_procedures:
            ...
            for role in procedure.get("roles", []):
                ...
                role_values[role_name] = {
                    "expected": role_value,
                    "doctor": {"id": role.get("id"), "name": role.get("name")},
                }
    ...
    def get_summary(self) -> Dict:
        ...
        invalid_value = sum(
            1 for r in self.comparison_results if r["status"] == "INVALID_VALUE"
        )
        not_in_cbhpm = sum(
            1 for r in self.comparison_results if r["status"] == "NOT_IN_CBHPM"
        )
        total_expected = sum(
            r["validation"]["expected_total"] for r in self.comparison_results
        )
        total_actual = sum(
            r["validation"]["actual_total"] for r in self.comparison_results
        )
